...
func Run(args []string) error {
	...
	if len(args) > 0 {
		...
		case "popup", "--popup":
			...
			if statePath := os.Getenv("TMUX_PARATOR_STATE"); statePath != "" {
				absoluteStatePath, err := filepath.Abs(statePath)
				if err != nil {
					return fmt.Errorf("resolve template memory path: %w", err)
				}
				popupCommand = "TMUX_PARATOR_STATE=" + shellQuote(absoluteStatePath) + " " + popupCommand
			}
...
Bindings can be changed under [ui.keys] in config.toml.`)
	...
	statePath, err := templatememory.Path()
	if err != nil {
		return fmt.Errorf("resolve template memory path: %w", err)
	}
	templateIDs := make([]string, 0, len(sessionCfg.Templates))
	for _, template := range sessionCfg.Templates {
		if template.Enabled && template.ID != "" {
			templateIDs = append(templateIDs, template.ID)
		}
	}
	memory, err := templatememory.Load(statePath, templateIDs)
	if err != nil {
		return err
	}
	model := ui.NewModelWithTemplateMemory(client, activeTheme, cfg.Roots, discovery.OptionsFromConfig(cfg.Discovery), cfg.PathSearch, cfg.UI.Glyphs, cfg.UI.GlyphColors, cfg.UI.Columns, cfg.UI.Keys, sessionCfg.Templates, memory, cfg.UI.Dialogs)
